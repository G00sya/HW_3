...
# def test_fit_model_training_mode(setup):
#     model, criterion, optimizer, train_iter, _ = setup
#
#     # We need to mock do_epoch to control its behavior
#     with patch('src.train.do_epoch', return_value=0.5) as mock_do_epoch:
#         # Reset any existing mock calls
#         model.train.reset_mock()
#
#         # Run the fit function
#         fit(model, criterion, optimizer, train_iter, epochs_count=1)
#
#         # Verify model.train(True) was called at least once
#         # We look for any call with True argument
#         assert any(call.args[0] is True for call in model.train.call_args_list)


# def test_fit_validation_mode(setup):
#     model, criterion, optimizer, train_iter, val_iter = setup
#
#     # Create a list to track train() calls
#     train_mode_calls = []
#
#     # Create a side effect function to track the modes
#     def track_train_mode(mode):
#         train_mode_calls.append(mode)
#         return None  # train() doesn't return anything
#
#     # Apply our tracker to the mock
#     model.train.side_effect = track_train_mode
#
#     # Mock do_epoch to return predictable losses
#     with patch('src.train.do_epoch', side_effect=[0.5, 0.4]):
#         # Run the fit function with validation
#         fit(model, criterion, optimizer, train_iter,
#             epochs_count=1, val_iter=val_iter)
#
#         # Verify the mode changes
#         # First call should be True (training), second False (validation)
#         assert train_mode_calls == [True, False]
